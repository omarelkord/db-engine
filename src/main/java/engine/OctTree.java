    ...
    public void insertInTree(Point point) {
        ...
        if (!divided) {
            ...
            if (items.size() < this.capacity) {
                Point clonePoint = new Point(point.x,point.y,point.z,point.pageReference.get(0));
                items.add(clonePoint);
                return;
            } else
                divide();
        }
...
//        System.out.println(child8.centre);
                ...
                if (boundary.isInRange((Comparable) item.x, (Comparable) child.boundary.front, (Comparable) child.boundary.back)) {
                    if (boundary.isInRange((Comparable) item.y, (Comparable) child.boundary.right, (Comparable) child.boundary.left)) {
                        if (boundary.isInRange((Comparable) item.z, (Comparable) child.boundary.top, (Comparable) child.boundary.bottom)) {
                            Point clonedPoint = new Point(item.x , item.y , item.z, item.pageReference);
                            child.items.add(clonedPoint);
                        }
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        Point point2 = new Point(10, 10, 10, 1);
        t.insertInTree(point2);
        Point point3 = new Point(90, null, null, 2);
        t.insertInTree(point3);
        Point point4 = new Point(60, 20, 20, 1);
        t.insertInTree(point4);
        Point point5 = new Point(5, 5, 5, 3);
...
//
//        System.out.println(new Date(((new Date(1735689600000L)).getTime() + 86400000)));
...
class Cube implements Serializable {
    ...
    public static String stringMean(String str1, String str2) {
        ...
        while(i<str1.length()&&i<str2.length()){
            ...
            int meanAscii = (int) Math.round((ascii1 + ascii2) / 2.0);
...
class Point implements Serializable {
    ...
    public Point(Object x, Object y, Object z, Vector<Integer> refs) {
        this.x = x;
        this.y = y;
        this.z = z;
        pageReference = new Vector<>();
        for(Integer ref: refs)
            pageReference.add(ref);
